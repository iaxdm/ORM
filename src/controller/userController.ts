...
export class UserController {
  ...
  async list(req: Request, res: Response) {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Ocorreu um erro inesperado ao lsitar os usuários" });
    }
  }
  ...
  async lsitById(req: Request, res: Response) {
    try {
      const id = Number(req.params.id);
      if (isNaN(id)) {
        res.status(400).json({ message: "ID inválido" });
      ...
      return res
        .status(500)
        .json({ error: "Ocorreu um erro inesperado ao lsitar o usuário" });
    }
  }
  ...
  async toggleActive(req: Request, res: Response) {
    try {
      ...
      return res.status(500).json({
        error: "Ocorreu um erro inesperado mudar o status do usuário",
      });
    }
  }
}
